...
10. `git push origin vX.Y.Z` — that tag only, never `--tags`, which would also push any unrelated local tag. Then WAIT for the Release workflow to finish (`gh run watch`). It refuses to publish if the tag and `CMakeLists.txt` disagree. It leaves the release as a draft.
11. `packaging/sign-release.sh vX.Y.Z` — detach-signs every asset and publishes the release.
12. Publish to the apt repo: `~/GIT/apt-ticpu-net/ingest.sh keyleds vX.Y.Z`.
13. Update the AUR package. AUR commits get no Co-Authored-By trailer.
